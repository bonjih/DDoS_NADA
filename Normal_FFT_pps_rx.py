...
df = pd.read_csv('data/test_pps_rx_1_month_epoch.csv', usecols=['Date', 'Utilisation pps Rx'], na_values=(0),
                 parse_dates=['Date'])
...
fftfreq = sp.fftpack.fftfreq(len(pps_psd), 1 / 10)  # 6 periods in 30 days
...
plt.title('Peak frequency (Normal)')
...
ax.set_xlabel('Frequency Bins (month)')
